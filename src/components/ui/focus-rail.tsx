...
export function FocusRail({
  ...
  return (
    <div
      className={cn(
        "group relative flex h-150 w-full flex-col overflow-hidden bg-neutral-950 text-white outline-none select-none overflow-x-hidden rounded-3xl",
        className
      )}
          ...
          {visibleIndices.map((offset) => {
            ...
            return (
              <motion.div
                key={absIndex}
                className={cn(
                  "absolute aspect-3/4 w-65 md:w-75 rounded-2xl border-t border-white/20 bg-neutral-900 shadow-2xl transition-shadow duration-300",
        ...
        <div className="mx-auto mt-12 flex w-full max-w-4xl flex-col items-center justify-between gap-6 md:flex-row pointer-events-auto">
          <div className="flex flex-1 flex-col items-center text-center md:items-start md:text-left h-32 justify-center">
            <AnimatePresence mode="wait">
              ...
              >
                {activeItem.meta && (
                  <span className="text-xs font-medium uppercase tracking-wider text-[#c4956a]">
                    {activeItem.meta}
                  </span>
                )}
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-1 rounded-full bg-neutral-900/80 p-1 ring-1 ring-white/10 backdrop-blur-md">
              ...
              <span className="min-w-10 text-center text-xs font-mono text-neutral-500">
                {activeIndex + 1} / {count}
              </span>
              <button
                onClick={handleNext}
